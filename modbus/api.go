...
type Client interface {
	...
	// WriteSingleDiscreteInput write a single output to either ON or OFF in a
	// remote device and returns output value.
	WriteSingleDiscreteInput(address, value uint16) (results []byte, err error)
	// WriteMultipleDiscreteInput forces each coil in a sequence of discret inputs to either
	// ON or OFF in a remote device and returns quantity of outputs.
	WriteMultipleDiscreteInput(address, quantity uint16, value []byte) (results []byte, err error)
	...
	// WriteSingleInputRegister writes a single input register in a remote
	// device and returns register value.
	WriteSingleInputRegister(address, value uint16) (results []byte, err error)
	// WriteMultipleInputRegisters writes a block of contiguous registers
	// (1 to 123 registers) in a remote device and returns quantity of
	// registers.
	WriteMultipleInputRegisters(address, quantity uint16, value []byte) (results []byte, err error)
}
