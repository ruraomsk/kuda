...
//  Output address        : 2 bytes
//  Output value          : 2 bytes
func (mb *client) WriteSingleDiscreteInput(address, value uint16) (results []byte, err error) {
	// The requested ON/OFF state can only be 0xFF00 and 0x0000
	if value != 0xFF00 && value != 0x0000 {
		err = fmt.Errorf("modbus: state '%v' must be either 0xFF00 (ON) or 0x0000 (OFF)", value)
		return
	...
	request := ProtocolDataUnit{
		FunctionCode: FuncCodeWriteSingleDiscreteInput,
		Data:         dataBlock(address, value),
	}
	response, err := mb.send(&request)
	if err != nil {
...
//  Register address      : 2 bytes
//  Register value        : 2 bytes
func (mb *client) WriteSingleInputRegister(address, value uint16) (results []byte, err error) {
	request := ProtocolDataUnit{
		FunctionCode: FuncCodeWriteSingleInputRegister,
		Data:         dataBlock(address, value),
	}
	response, err := mb.send(&request)
	if err != nil {
...
//  Starting address      : 2 bytes
//  Quantity of outputs   : 2 bytes
func (mb *client) WriteMultipleDiscreteInput(address, quantity uint16, value []byte) (results []byte, err error) {
	if quantity < 1 || quantity > 1968 {
		err = fmt.Errorf("modbus: quantity '%v' must be between '%v' and '%v',", quantity, 1, 1968)
		return
	}
	request := ProtocolDataUnit{
		FunctionCode: FuncCodeWriteMultipleDiscreteInput,
		Data:         dataBlockSuffix(value, address, quantity),
	}
	response, err := mb.send(&request)
	if err != nil {
...
//  Starting address      : 2 bytes
//  Quantity of registers : 2 bytes
func (mb *client) WriteMultipleInputRegisters(address, quantity uint16, value []byte) (results []byte, err error) {
	if quantity < 1 || quantity > 123 {
		err = fmt.Errorf("modbus: quantity '%v' must be between '%v' and '%v',", quantity, 1, 123)
		return
	}
	request := ProtocolDataUnit{
		FunctionCode: FuncCodeWriteMultipleInputRegisters,
		Data:         dataBlockSuffix(value, address, quantity),
	}
	response, err := mb.send(&request)
	if err != nil {
