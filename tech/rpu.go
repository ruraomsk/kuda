...
func WorkRPU(c *bin.CMK, cs []*Commander, CommandFlow chan bin.PhaseCommand) {
	...
	coms[0] = &Commander{Level: 0, Responce: make(chan bin.ResponcePhase), CommandWait: bin.PhaseCommand{Phase: 1, PromTakt: true}}
	for _, v := range cs {
		coms[v.Level] = v
	}
		...
		case <-seekLevel.C:
			if phaseNow != 9 {
				...
				if lev != 0 {
					...
					if lev == 0 && levelNow != 0 {
						// fmt.Println("need start cycle")
						coms[0].CommandWait = bin.PhaseCommand{Level: 0, Phase: 1, PromTakt: true}
				...
				if send {
					lc := coms[lev]
					commands <- bin.PhaseCommand{Level: levelNow, Phase: lc.CommandWait.Phase, PromTakt: cmk.GetBaseOrUniver(phaseNow, lc.CommandWait.Phase)}
				}
			}
		...
		case <-ctrlCycle.C:
			if levelNow == 0 {
				...
				if phaseNow != cmk.RPUs[0].Phases[0].Phase {
					...
					CommandFlow <- bin.PhaseCommand{Level: 0, Phase: phaseNow, PromTakt: true}
				}
			}
		case <-ctrlPhase.C:
			if levelNow == 0 {
				...
				CommandFlow <- bin.PhaseCommand{Phase: phaseNow, PromTakt: cmk.GetBaseOrUniver(phaseOld, phaseNow)}
			}
		case resp := <-responce:
			if resp.OsStop {
